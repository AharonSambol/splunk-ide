...
const { upsertStanza } = require('../objects/conf-stanza');
...
const { fetchSavedSearchStanza: defaultFetchSavedSearchStanza } = require('../objects/splunk-rest');
const { readHeadConf } = require('./object-io');
...
async function syncRemoteBranch(git, { remoteName = 'origin', sharedBranch }) {
    ...
    let mergeBase = '';
    try {
        mergeBase = (await git.raw(['merge-base', local, remote])).trim();
    } catch {
        mergeBase = '';
    }
    if (!mergeBase) {
        await git.reset(['--mixed', remoteRef]);
        return { ok: true, merged: true, resetUnrelated: true };
    }
