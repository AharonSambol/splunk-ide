...
async function listVersions(git, relativePath, maxCount = 30) {
    ...
    try {
        raw = await git.raw([
            ...
            `--max-count=${maxCount}`,
            '--pretty=format:COMMIT:%H%x00%an%x00%ad%x00%s',
            '--date=iso-strict',
            '--', normalized
        ]);
    } catch {
    ...
    for (const block of blocks) {
        ...
        const [hash, author, date, message] = lines[0].split('\0');
...
async function restoreVersion(git, relativePath, hash) {
    ...
    const { hasChanges } = await getFileStatus(git, normalized);
    if (hasChanges) {
        await saveVersion(git, normalized, `Auto-save before restore to ${hash.substring(0, 7)}`);
    }
