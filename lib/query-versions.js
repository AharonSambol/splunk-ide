 ...
 */
async function commitFileOnParent(git, relativePath, parentHash, message) {
    const gitDir = (await git.revparse(['--absolute-git-dir'])).trim();
...
async function saveVersion(git, relativePath, message, parentHash, options = {}) {
    ...
    if (isOffHeadSave) {
        const commitHash = await commitFileOnParent(git, normalized, parentHash, commitMessage);
