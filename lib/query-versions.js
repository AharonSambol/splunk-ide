 ...
 */
async function listVersions(git, relativePath, maxCount = 30) {
    const normalized = relativePath.replace(/\\/g, '/');
    const isRepo = await git.checkIsRepo('root');
    if (!isRepo) {
        return [];
    ...
    for (const block of blocks) {
        ...
        const [hash, author, date, message, body = ''] = lines[0].split('\0');
 ...
 */
async function saveVersion(git, relativePath, message, parentHash) {
    ...
    const commitMessage = parentHash
        ? `${message}\n\nQuery-Parent: ${parentHash}`
        : message;
 ...
 */
async function restoreVersion(git, relativePath, hash, parentHash) {
    const normalized = relativePath.replace(/\\/g, '/');
    const { hasChanges } = await getFileStatus(git, normalized);
    if (hasChanges) {
        await saveVersion(git, normalized, `Auto-save before restore to ${hash.substring(0, 7)}`, parentHash);
    }
