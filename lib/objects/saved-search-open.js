...
const { ensureRepo, saveVersion } = require('../git/query-versions');
const {
    ensureRemote,
    fetchSharedHistory,
    hasRemoteBranch,
    alignSharedBranchWithRemote
} = require('../git/git-sync');
const { listStanzaDraftsForConf, recomposeWorktree } = require('../git/stanza-drafts');
const { fetchSavedSearchStanza: defaultFetchSavedSearchStanza } = require('./splunk-rest');
const { checkoutSharedBranch, getHeadHash, latestCommitWithFile, restoreFromGit, readHeadConf } = require('../git/object-io');
