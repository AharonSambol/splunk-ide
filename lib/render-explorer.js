...
function renderFolderNode(document, folder, activeFileId, handlers) {
    ...
    details.dataset.folderPath = folder.path;
    details.open = handlers.isFolderOpen?.(folder.path) !== false;
    ...
    details.addEventListener('toggle', () => {
        handlers.onFolderToggle?.(folder.path, details.open);
    });
    ...
    details.addEventListener('drop', event => {
        ...
        if (fileId) {
            handlers.onFileDrop?.(fileId, folder.path);
        }
    });
    return details;
}

function renderExplorer(container, tree, { activeFileId, isEmpty, hideRootLabel } = {}, handlers = {}) {
